...
- **`record`** (optional) — arrives with the **Phase-B resume** (the same late-arrival slot as the
  ...
  Phase B captures each test case as a separate video clip and uploads it (see Phase B steps 3–4).
  Absent or declined ⇒ no recording — behavior unchanged.
...
5. **Post results to the PR** — `gh pr comment <ref> --body '<results>'`, where `<results>` is a
   ...
   **Overall: <passed>/<total> passed** ✅|❌
   ...
   When a recording was made, add one labeled line per finalized clip under the verdict line:
...
6. **Report** — return the **same verdict-line + table** as your final message (identical structure to
   ...
   Close **every** browser instance (`playwright-cli -s=<session> close` per instance) at the end.
