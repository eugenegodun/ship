...
|---|---|
| Spec approved | `spawn_agent` planner with approved spec |
...
| PR and QA plan ready | Surface plan and request approval; end turn at GATE 3 |
...
- After “Approved” at GATE 2: `spawn_agent({task_name: "lex_1398_implementator",
...
Inputs: ticket key; optional `--spec` and `--record`. Ask for a missing ticket or unexplained extra
...
|---|---|---|
| Spec | `ship-spec-agent` | gpt-5.6-sol / xhigh |
...
Retain ticket, flags, approved spec/plan, role identities, worktree/branch, verification evidence,
...
### 1. Optional spec — GATE 1

Only with `--spec`: dispatch spec role with ticket and user context, then wait. Surface the returned
spec content verbatim and ask for approval. Stop until the verdict. Changes go to the same spec
agent. Approval retains the spec and immediately dispatches the planner; there is no spec Phase B.
Without `--spec`, skip to planning.

### 2. Plan — GATE 2

Dispatch planner with ticket, user context, and approved spec inline when present. Wait. Surface
...
Brief the implementer with ticket, approved plan inline, approved spec inline when present, and user
constraints. It creates an isolated worktree and ticket-named branch (respect explicit user naming/base).
Only spec-mode runs persist `specs/<TICKET>/*.md`. Require changed files, worktree path, branch,
...
### 4. Review and fixes — maximum three review rounds
...
### 6. QA plan — GATE 3; then execution
...
### 7–8. Results and best-effort retrospectives
...
Commit/Push/Draft PR, QA, Final report, Insights retro; include Spec when used. Keep every row,
