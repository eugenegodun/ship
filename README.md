...
[`evals/`](evals/) — 123 cases in five tiers. GitHub Actions runs the first four tiers on
PRs touching `plugins/ship/**` or `evals/**`; end-to-end cases run nightly or manually:

| Tier | Cases | What it checks |
|------|-------|----------------|
| Unit | 71 | The harness itself — artifact loading, tool schemas, the turn simulator, the Codex role generator/installer, and version invariants. No model calls. |
