...
- Role table (spec): spec `gpt-5.6-sol`/`xhigh`/`read-only`; planner `gpt-5.6-sol`/`xhigh`/`read-only`; implementator `gpt-5.6-terra`/`high`/`workspace-write`; reviewer `gpt-5.6-sol`/`xhigh`/`workspace-write`; qa `gpt-5.6-terra`/`medium`/`workspace-write`; git `gpt-5.6-luna`/`low`/`workspace-write`.
